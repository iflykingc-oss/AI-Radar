...
export function RegisterModal({ open, onOpenChange, onSwitchToLogin }: RegisterModalProps) {
  ...
  if (success) {
    return (
      <Dialog open onOpenChange={(o) => { if (!o) onOpenChange(false); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t('registration_success')}</DialogTitle>
          </DialogHeader>
          <Card className="border-none shadow-none">
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <Button onClick={() => onOpenChange(false)}>{t('got_it')}</Button>
            </CardContent>
          </Card>
        </DialogContent>
      </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t('register_title')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <p className="text-center text-sm text-muted-foreground">
            {t('already_account')}{' '}
            <button className="text-primary hover:underline" onClick={() => { onOpenChange(false); onSwitchToLogin(); }}>
              {t('log_in')}
            </button>
          </p>
        </div>
